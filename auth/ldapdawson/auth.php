 ...
 */
class auth_plugin_ldapdawson extends auth_plugin_base {
    ...
    function init_plugin($authtype) {
        ...
        //Added by Jason. I'll used the enrol DB for first and last names,
        //because those fields in LDAP can't handle accents, etc.
        $this->enroldb = null;
    }
     ...
     */
    function auth_plugin_ldapdawson() {
        $this->authtype = 'ldapdawson';
        $this->roleauth = 'auth_ldapdawson';
        $this->errorlogtag = '[AUTH LDAPDAWSON] ';
    ...
    function user_login($username, $password) {
        ...
        if (! function_exists('ldap_bind')) {
            print_error('auth_ldapdawsonnotinstalled', 'auth_ldapdawson');
            return false;
        }
        ...
        if ($ldap_login) {
            ...
            //Check if we're using the 'cegep' enrolment plugin.
            //If we are, get the courses that this user teaches and pass them
            //to the enrol_cegep_plugin::sync_courses function.
            //
            //Also pass them to enrol_cegep_plugin::sync_merged_courses, which
            //creates a merged course by default.
            if (enrol_is_enabled('cegep')) {
                require_once("$CFG->dirroot/enrol/cegep/locallib.php");
                if ($enrol_courses = enrol_cegep_get_courses($username)) {
                    $enrol = enrol_get_plugin('cegep');
                    $enrol->sync_courses($enrol_courses);
                    $enrol->sync_merged_courses($enrol_courses);
                    try {
                        $enrol->sync_user_enrolments($username);
                    }
        ...
        switch ($this->config->user_type)  {
            ...
            case 'ad':
                ...
                if (!function_exists('mb_convert_encoding')) {
                    print_error('auth_ldapdawson_no_mbstring', 'auth_ldapdawson');
                }

                // Check for invalid sAMAccountName characters.
                if (preg_match('#[/\\[\]:;|=,+*?<>@"]#', $extusername)) {
                    print_error ('auth_ldapdawson_ad_invalidchars', 'auth_ldapdawson');
                }
                ...
                if (!ldap_add($ldapconnection, $userdn, $newuser)) {
                    print_error('auth_ldapdawson_ad_create_req', 'auth_ldapdawson');
                }
                ...
                break;
            default:
               print_error('auth_ldapdawson_unsupportedusertype', 'auth_ldapdawson', '', $this->config->user_type_name);
        }
        $this->ldap_close();
        return $uadd;
    }
    ...
    function user_signup($user, $notify=true) {
        ...
        if ($this->user_exists($user->username)) {
            print_error('auth_ldapdawson_user_exists', 'auth_ldapdawson');
        }
        ...
        if (! $this->user_create($user, $plainslashedpassword)) {
            print_error('auth_ldapdawson_create_error', 'auth_ldapdawson');
        }
        ...
        if ($notify) {
            ...
            $PAGE->set_url('/auth/ldapdawson/auth.php');
    ...
    function password_expire($username) {
        ...
        if ($sr)  {
            ...
            error_log($this->errorlogtag.get_string('didtfindexpiretime', 'auth_ldapdawson'));
        }
    ...
    function sync_users($do_updates=true) {
        ...
        print_string('connectingldap', 'auth_ldapdawson');
        $ldapconnection = $this->ldap_connect();
    ...
    /// Define table user to be created
        ...
        print_string('creatingtemptable', 'auth_ldapdawson', 'tmp_extuser');
        ...
        $nocontexts = explode(';', $this->config->nocontexts);
        ...
        foreach ($contexts as $context) {
            ...
            if ($entry = @ldap_first_entry($ldapconnection, $ldap_result)) {
                do {
                    ...
                    //Check blacklisted contexts and skip the user if need be.
                    foreach ($nocontexts as $nocontext) {
                        if (stripos($userdn, $nocontext) !== false) {
                            $validentry = false;
                            break;
                        }
                    }
        ...
        if ($count < 1) {
            print_string('didntgetusersfromldap', 'auth_ldapdawson');
            exit;
        } else {
            print_string('gotcountrecordsfromldap', 'auth_ldapdawson', $count);
        }
...
/// User removal
        ...
        if ($this->config->removeuser !== AUTH_REMOVEUSER_KEEP) {
            ...
            if (!empty($remove_users)) {
                print_string('userentriestoremove', 'auth_ldapdawson', count($remove_users));

                foreach ($remove_users as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        try {
                            $userdeleted = delete_user($user);
                        }
                        catch (dml_write_exception $e) {
                            echo "\tError deleting user: $user->username\n";
                            echo "\tTrying again...\n";

                            //Try again, methinks this is a duplicate key issue
                            //because things are going to fast. By the time we get
                            //here, maybe a nanosecond has passed and it'll succeed.
                            sleep(1);
                            try {
                                $userdeleted = delete_user($user);
                            }
                            catch (dml_write_exception $e) {
                                echo "\t\tERROR Deleting user AGAIN.\n";
                            }
                        }
                ...
                print_string('nouserentriestoremove', 'auth_ldapdawson');
            }
...
/// Revive suspended users
        if (!empty($this->config->removeuser) and $this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
            ...
            if (!empty($revive_users)) {
                print_string('userentriestorevive', 'auth_ldapdawson', count($revive_users));
                ...
                print_string('nouserentriestorevive', 'auth_ldapdawson');
            }
...
/// User Updates - time-consuming (optional)
        if ($do_updates) {
            ...
            print_string('noupdatestobedone', 'auth_ldapdawson');
        }
        if ($do_updates and !empty($updatekeys)) { // run updates only if relevant
            ...
            if (!empty($users)) {
                print_string('userentriestoupdate', 'auth_ldapdawson', count($users));
...
/// User Additions
        ...
        if (!empty($add_users)) {
            print_string('userentriestoadd', 'auth_ldapdawson', count($add_users));
            ...
            foreach ($add_users as $user) {
                ...
                //Get the firstname and lastname fields from the enroldb, since
                //ldap is messed up and can't handle special characters.
                $query = "SELECT firstname, lastname FROM `enrolment` WHERE userno='$user->username' LIMIT 1";
                if ($rs = $this->enroldb->Execute($query)) {
                    if (!$rs->EOF) {
                        while ($fields = $rs->FetchRow()) {
                            if (strlen($fields['firstname']) > 0) {
                                $user->firstname = $fields['firstname'];
                            }

                            if (strlen($fields['lastname']) > 0) {
            ...
            print_string('nouserstobeadded', 'auth_ldapdawson');
        }
        ...
        if ($newinfo = $this->get_userinfo($username)) {
            ...
            foreach ($updatekeys as $key) {
                if (isset($newinfo[$key])) {
                    ...
                    //Names in LDAP are messed up. Get them from the external database.
                    $extset = false;
                    if ($key == 'firstname' || $key == 'lastname') {

                        $query = "SELECT $key FROM `enrolment` WHERE userno='$user->idnumber' LIMIT 1";
                        if ($rs = $this->enroldb->Execute($query)) {
                            if (!$rs->EOF) {
                                while ($fields = $rs->FetchRow()) {
    ...
    function user_activate($username) {
        ...
        switch ($this->config->user_type)  {
            ...
            case 'ad':
                ...
                break;
            default:
                print_error('user_activatenotsupportusertype', 'auth_ldapdawson', '', $this->config->user_type_name);
        }
    ...
    function user_update($olduser, $newuser) {
        ...
        if (isset($olduser->username) and isset($newuser->username) and $olduser->username != $newuser->username) {
            error_log($this->errorlogtag.get_string('renamingnotallowed', 'auth_ldapdawson'));
            return false;
        }
        ...
        if ($user_info_result) {
            ...
            if (empty($user_entry)) {
                ...
                error_log($this->errorlogtag.get_string('updateusernotfound', 'auth_ldapdawson',
                                                          array('userdn'=>$user_dn,
                                                                'attribs'=>$attribs)));
                return false; // old user not found!
            } else if (count($user_entry) > 1) {
                error_log($this->errorlogtag.get_string('morethanoneuser', 'auth_ldapdawson'));
                return false;
            }
                ...
                if (isset($olduser->$key) and isset($newuser->$key)
                  ...
                  and !empty($this->config->{'field_updateremote_'. $key})) {
                        ...
                        if (!$ambiguous) {
                            ...
                            if ($nuvalue !== $ldapvalue) {
                                ...
                                if (@ldap_modify($ldapconnection, $user_dn, array($ldapkey => $nuvalue))) {
                                    ...
                                    error_log($this->errorlogtag.get_string ('updateremfail', 'auth_ldapdawson',
                    ...
                    if ($ambiguous and !$changed) {
                        error_log($this->errorlogtag.get_string ('updateremfailamb', 'auth_ldapdawson',
                                                                 array('key'=>$key,
                                                                       'ouvalue'=>$ouvalue,
                                                                       'nuvalue'=>$nuvalue)));
                    }
            ...
            error_log($this->errorlogtag.get_string ('usernotfound', 'auth_ldapdawson'));
            $this->ldap_close();
            return false;
        }
    ...
    function user_update_password($user, $newpassword) {
        ...
        if (!$user_dn) {
            error_log($this->errorlogtag.get_string ('nodnforusername', 'auth_ldapdawson', $user->username));
            return false;
        }
        ...
        switch ($this->config->user_type) {
            case 'edir':
                ...
                if (!$result) {
                    error_log($this->errorlogtag.get_string ('updatepasserror', 'auth_ldapdawson',
                                                               array('errno'=>ldap_errno($ldapconnection),
                                                                     'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                }
                ...
                if ($sr) {
                    ...
                    if (!empty($info[$this->config->expireattr][0])) {
                        ...
                        if (!$result) {
                            error_log($this->errorlogtag.get_string ('updatepasserrorexpiregrace', 'auth_ldapdawson',
                                                                       array('errno'=>ldap_errno($ldapconnection),
                                                                             'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                        }
                    }
                    ...
                    error_log($this->errorlogtag.get_string ('updatepasserrorexpire', 'auth_ldapdawson',
                                                             array('errno'=>ldap_errno($ldapconnection),
                                                                   'errstring'=>ldap_err2str(ldap_errno($ldapconnection)))));
                }
                break;
            ...
            case 'ad':
                ...
                // double quotes. See http://support.microsoft.com/?kbid=269190
                if (!function_exists('mb_convert_encoding')) {
                    error_log($this->errorlogtag.get_string ('needmbstring', 'auth_ldapdawson'));
                    return false;
                }
    ...
    function ldap_expirationtime2unix ($time, $ldapconnection, $user_dn) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'ad':
                ...
                break;
            default:
                print_error('auth_ldapdawson_usertypeundefined', 'auth_ldapdawson');
        }
        return $result;
    }
    ...
    function ldap_unix2expirationtime($time) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'rfc2307bis':
                ...
                break;
            default:
                print_error('auth_ldapdawson_usertypeundefined2', 'auth_ldapdawson');
        }
        return $result;
    ...
    function loginpage_hook() {
        ...
        if (($_SERVER['REQUEST_METHOD'] === 'GET'         // Only on initial GET of loginpage
            ...
            if(!empty($this->config->ntlmsso_ie_fastpath)) {
                ...
                if(check_browser_version('MSIE')) {
                    $sesskey = sesskey();
                    redirect($CFG->wwwroot.'/auth/ldapdawson/ntlmsso_magic.php?sesskey='.$sesskey);
                } else {
                    redirect($CFG->httpswwwroot.'/login/index.php?authldapdawson_skipntlmsso=1');
                }
            } else {
                redirect($CFG->wwwroot.'/auth/ldapdawson/ntlmsso_attempt.php');
            }
        }
        ...
        if (empty($SESSION->wantsurl)
            && (get_referer() == $CFG->httpswwwroot.'/auth/ldapdawson/ntlmsso_finish.php')) {
        ...
        if (isset($_SERVER['REMOTE_USER']) && !empty($_SERVER['REMOTE_USER'])) {
            ...
            switch ($this->config->ntlmsso_type) {
                ...
                case 'kerberos':
                    ...
                    break;
                default:
                    error_log($this->errorlogtag.get_string ('ntlmsso_unknowntype', 'auth_ldapdawson'));
    ...
    function config_form($config, $err, $user_fields) {
        ...
        if (!function_exists('ldap_connect')) { // Is php-ldap really there?
            echo $OUTPUT->notification(get_string('auth_ldapdawson_noextension', 'auth_ldapdawson'));
            return;
        }

        include($CFG->dirroot.'/auth/ldapdawson/config.html');
    }
    ...
    function process_config($config) {
        ...
        set_config('nocontexts', trim($config->nocontexts), $this->pluginconfig);
        ...
        set_config('ntlmsso_type', $config->ntlmsso_type, 'auth/ldapdawson');
    ...
    function ldap_get_ad_pwdexpire($pwdlastset, $ldapconn, $user_dn){
        ...
        if (!function_exists('bcsub')) {
            error_log($this->errorlogtag.get_string ('needbcmath', 'auth_ldapdawson'));
            return 0;
        }
        ...
        $sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)',
                        array('userAccountControl'));
        if (!$sr) {
            error_log($this->errorlogtag.get_string ('useracctctrlerror', 'auth_ldapdawson', $user_dn));
        ...
        if (!$sr) {
            error_log($this->errorlogtag.get_string ('rootdseerror', 'auth_ldapdawson'));
            return 0;
        }
    ...
    function ldap_connect() {
        ...
        print_error('auth_ldapdawson_noconnect_all', 'auth_ldapdawson', '', $debuginfo);
    }
    ...
    /**
     * Initializes this->enroldb. If it's already been initialized,
     * just return it.
     */
    function init_enroldb() {
        if (!is_null($this->enroldb)) {
            return $this->enroldb;
        }

        global $CFG;

        $enrol_config = get_config('enrol_cegep');

        require_once($CFG->libdir.'/adodb/adodb.inc.php');

        // Connect to the external database (forcing new connection)
        $this->enroldb = ADONewConnection($enrol_config->dbtype);
        if ($enrol_config->debugdb) {
            $this->enroldb->debug = true;
            ob_start(); //start output buffer to allow later use of the page headers
        }

        $this->enroldb->Connect($enrol_config->dbhost, $enrol_config->dbuser, $enrol_config->dbpass, $enrol_config->dbname, true);
        $this->enroldb->SetFetchMode(ADODB_FETCH_ASSOC);
        if ($enrol_config->dbsetupsql) {
            $this->enroldb->Execute($enrol_config->dbsetupsql);
        }
    }
