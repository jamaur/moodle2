...
function xmldb_auth_ldapdawson_install() {
    global $CFG, $DB;
    ...
    if (get_config('auth/ldapdawson', 'preventpassindb')) {
        $DB->set_field('user', 'password', 'not cached', array('auth'=>'ldapdawson'));
    }
    ...
    if ($ldap_version = get_config('auth/ldapdawson', 'version')) {
        set_config('ldap_version', $ldap_version, 'auth/ldapdawson');
        unset_config('version', 'auth/ldapdawson');
    }
}
