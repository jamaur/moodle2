...
/**
 * Cegep enrolment plugin implementation.
 * @author  Jason Maur - based on Petr Skoda's database plugin
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_cegep_plugin extends enrol_plugin {
    /**
    ...
    public function sync_user_enrolments($user) {
        ...
        if ($rs = $extdb->Execute($sql)) {
            if (!$rs->EOF) {
                while ($fields = $rs->FetchRow()) {
                    ...
                    if ($instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'cegep'), '*', IGNORE_MULTIPLE)) {
        ...
        foreach ($enrols as $courseid => $roles) {
            ...
            $current = $DB->get_records('role_assignments', array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_cegep', 'itemid'=>$instance->id), '', 'id, roleid');
            ...
            foreach ($current as $r) {
                if (in_array($r->id, $roles)) {
                    ...
                    role_unassign($r->roleid, $user->id, $context->id, 'enrol_cegep', $instance->id);
                }
            }
            foreach ($roles as $rid) {
                if (!isset($existing[$rid])) {
                    role_assign($rid, $user->id, $context->id, 'enrol_cegep', $instance->id);
                }
            }
        }
        ...
        $sql = "SELECT e.*, c.visible AS cvisible, ue.status AS ustatus
                 ...
                 WHERE ue.userid = :userid AND e.enrol = 'cegep'";
        ...
        foreach ($rs as $instance) {
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                    role_unassign_all(array('contextid'=>$context->id, 'userid'=>$user->id, 'component'=>'enrol_cegep', 'itemid'=>$instance->id));
                }
            }
        }
        $rs->close();
    ...
    public function sync_enrolments() {
        ...
        $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping, e.id AS enrolid
                  FROM {course} c
                  JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'cegep')";
        ...
        $sql = "SELECT c.id, c.visible, c.$localcoursefield AS mapping
                  FROM {course} c
             LEFT JOIN {enrol} e ON (e.courseid = c.id AND e.enrol = 'cegep')
        ...
        foreach ($existing as $course) {
            ...
            $sql = "SELECT u.$localuserfield AS mapping, u.id, ue.status, ue.userid, ra.roleid
                      ...
                      JOIN {role_assignments} ra ON (ra.userid = u.id AND ra.itemid = ue.enrolid AND ra.component = 'enrol_cegep')
                     WHERE u.deleted = 0";
            ...
            foreach ($requested_roles as $userid=>$userroles) {
                ...
                foreach($current_roles[$userid] as $cr) {
                    if (empty($userroles[$cr])) {
                        role_unassign($cr, $userid, $context->id, 'enrol_cegep', $instance->id);
            ...
            if ($unenrolaction == ENROL_EXT_REMOVED_UNENROL) {
                ...
                foreach ($current_status as $userid=>$status) {
                    ...
                    if ($unenrolaction == ENROL_EXT_REMOVED_SUSPENDNOROLES) {
                        role_unassign_all(array('contextid'=>$context->id, 'userid'=>$userid, 'component'=>'enrol_cegep', 'itemid'=>$instance->id));
                    }
                }
            }
        }
     ...
     * @return void
     */
    public function sync_courses($createcourses = array()) {
        global $CFG, $DB;
        ...
        //No courses passed? Do it for everything.
        if (!is_array($createcourses) || count($createcourses) <= 0) {
            $extdb = $this->db_init();
        ...
        if ($createcourses) {
            ...
            foreach ($createcourses as $fields) {
                if (($existing = $DB->get_record('course', array('shortname'=>$fields->shortname)))) {
                    continue;
                }
    ...
    /**
     * Takes an array of courses, and checks that they have
     * a corresponding "parent" course, if necessary. If the
     * course doesn't have any siblings, then there's no need
     * to create a merged course. This function uses the mydawson
     * block functions extensively.
     */

    public function sync_merged_courses($createcourses) {
        global $DB, $USER, $CFG;
        require_once($CFG->dirroot . '/blocks/mydawson/lib.php');

        $sorted_courses = mydawson_sort_courses($createcourses);

        foreach ($sorted_courses as $session => $codes) {
            foreach ($codes as $code => $children) {

                //we don't care about singletons--that is, no need to merge one
                //course unto itself.
                if (count($children) <= 1) {
                    continue;
                }
                ...
                foreach ($children as $c) {
                    $sections[] = mydawson_extract_section($c->idnumber);
                    if (($child_id = $DB->get_record('course', array('idnumber'=>$c->idnumber), 'id'))) {
                        $c->id = $child_id->id; //for use later.
                        $children_to_add[] = $child_id->id;
                    }
                }
                ...
                $section_string = implode("+", $sections);
                $newidnumber  = $code . '-' . $section_string . '-' . $session;

                $section_string_full = get_string('section', 'block_mydawson') . (count($sections) > 1 ? 's' : '') . ' ' . implode(", ", $sections);
                ...
                if (($newcourse = $DB->get_record('course', array('idnumber'=>$newidnumber), '*'))) {
                    ...
                    $newcourse = clone($template);
                    $newcourse->fullname  = $children[0]->fullname;
                    $newcourse->shortname  = $children[0]->fullname . " " . $section_string_full . " - " . mydawson_session_to_string($session);
                    $newcourse->idnumber  = $code . '-' . $section_string . '-' . $session;

                    $newcourse->category  = $children[0]->category;
                    $newcourse->visible = 1;
